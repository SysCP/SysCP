...
class Syscp_Hooks_Traffic extends Syscp_BaseHook 
{
	 ...
	 * @access  public
	 * 
	 * @return  Org_Syscp_Core_Hooks_Traffic
	 */
	 ...
	 */
	function cronCalcTraffic( $params = array() )
	{
		...
		// first of all, we need to reschedule another traffic run in 5mins
		$log->info( '-- cronCalcTraffic: Rescheduling traffic run, for next cron run...' );
		$this->_hooks->schedule( $this->FILE, $this->CLASS, 'cronCalcTraffic', array() );
		
		$log->info( '-- cronCalcTraffic: Checking if we need to calculate traffic and diskspace...');
		// check if there has already been one traffic run today
		if($config->get('system.last_traffic_run') != date('dmy'))
		{
			$log->info( '-- cronCalcTraffic: Starting traffic accounting' );
			...
			while( true === ($row = $db->fetch_array($result)) )
			{
				...
				$log->info( sprintf( '-- cronCalcTraffic: counting http traffic for %s',
				                     $row['loginname'] ) );
				...
				$log->info( sprintf( '-- cronCalcTraffic: counting ftp traffic for %s', 
				                     $row['loginname'] ) );
				...
				{
					$log->info( sprintf( '--cronCalcTraffic: creating traffic sum for %s',
					                     $row['loginname'] ) );
				...
				{
					$log->info( sprintf( '--cronCalcTraffic: creating traffic (new month) sum for %s',
					                     $row['loginname'] ) );
				...
				$log->info( sprintf( '-- cronCalcTraffic: counting diskspace for webs of %s', 
				                     $row['loginname'] ) );
				...
				$log->info( sprintf( '-- cronCalcTraffic: counting diskspace for mails of %s', 
				                     $row['loginname'] ) );
				                     
				// init the mailspace usage with zero
				$emailusage = 0;
				// execute du and catch the result 
				$back = safe_exec('du -s "'.$config->get('system.vmail_homedir').$row['loginname'].'"');
				// iterate the result 
				foreach($back as $backrow)
				{
				...
				$log->info( sprintf( '-- cronCalcTraffic: counting diskspace for databases of %s', 
				                     $row['loginname'] ) );
