<?php
		...
		{
			if($this->User['email_forwarders_used'] < $this->User['email_forwarders'] || $this->User['email_forwarders'] == '-1')
			{
				...
				if(isset($result['email']) && $result['email']!='')
				{
					if(isset($_POST['send']) && $_POST['send']=='send')
					{
						...
						if($destination == '')
						{
							$this->TemplateHandler->showError('destinationnonexist');
							return false;
						}
						elseif(!verify_email($destination))
						{
							$this->TemplateHandler->showError('destinationiswrong',$destination);
							return false;
						}
						elseif($destination == $result['email'])
						{
							$this->TemplateHandler->showError('destinationalreadyexistasmail',$destination);
							return false;
						}
						elseif(in_array ( $destination , $result['destination_array'] ))
						{
							$this->TemplateHandler->showError('destinationalreadyexist',$destination);
							return false;
						}
						else
						{
