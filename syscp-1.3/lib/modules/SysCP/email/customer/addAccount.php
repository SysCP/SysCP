<?php
		...
		{
			if($this->User['email_accounts_used'] < $this->User['email_accounts'] || $this->User['email_accounts'] == '-1')
			{
				...
				if(isset($result['email']) && $result['email']!='' && $result['popaccountid'] == '0')
				{
					if(isset($_POST['send']) && $_POST['send']=='send')
					{
						...
						else
						{
							...
							$query = 'INSERT INTO `'.TABLE_MAIL_USERS.'` '
							       ...
							       . '    `homedir`      =\''.$homedir.'\', '
							       . '    `maildir`      =\'/'.$email_full.'/\', '
							       . '    `uid`          =\''.$this->ConfigHandler->get('system.vmail_uid').'\', '
							       . '    `gid`          =\''.$this->ConfigHandler->get('system.vmail_gid').'\', '
							...
							$mail_subject=_html_entity_decode(replace_variables((($result['value']!='') ? $result['value'] : $this->L10nHandler->get('mails.pop_success.subject')),$replace_arr));
							...
							$mail_body=_html_entity_decode(replace_variables((($result['value']!='') ? $result['value'] : $this->L10nHandler->get('mails.pop_success')),$replace_arr));
			...
			{
				$this->TemplateHandler->showError('allresourcesused');
				return false;
			}
		}
