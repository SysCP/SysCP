<?php
			if($this->User['emails_used'] < $this->User['emails'] || $this->User['emails'] == '-1')
			{
				if(isset($_POST['send']) && $_POST['send']=='send')
				{
					...
					{
						$this->TemplateHandler->showError('emailexistalready',$email_full);
						return false;
					}
					elseif(!verify_email($email_part . '@' . $domain))
					{
						$this->TemplateHandler->showError('emailiswrong',$email_full);
						return false;
					}
					...
					{
						$this->TemplateHandler->showError('youhavealreadyacatchallforthisdomain');
						return false;
					}
					else
					{
				...
				else
				{
					...
					$isCatchall = array( 1 => $this->L10nHandler->get('panel.yes'),
