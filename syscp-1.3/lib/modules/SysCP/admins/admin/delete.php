	...
	if($this->User['change_serversettings'] == '1' )
		{
			...
			if($result['loginname']!='')
			{
				...
				{
					$this->DatabaseHandler->query("DELETE FROM `".TABLE_PANEL_ADMINS."` WHERE `adminid`='".$this->ConfigHandler->get('env.id')."'");
					$this->DatabaseHandler->query("DELETE FROM `".TABLE_PANEL_TRAFFIC_ADMINS."` WHERE `adminid`='".$this->ConfigHandler->get('env.id')."'");
					$this->DatabaseHandler->query("UPDATE `".TABLE_PANEL_CUSTOMERS."` SET `adminid` = '1' WHERE `adminid` = '".$this->ConfigHandler->get('env.id')."'");
					$this->DatabaseHandler->query("UPDATE `".TABLE_PANEL_DOMAINS."` SET `adminid` = '1' WHERE `adminid` = '".$this->ConfigHandler->get('env.id')."'");
					/**
				...
				{
					$this->TemplateHandler->showQuestion('admin_admin_reallydelete',
					                                     array('module' => 'admins',
