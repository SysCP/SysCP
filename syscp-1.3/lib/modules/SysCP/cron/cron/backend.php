...
// +----------------------------------------------------------------
	...
	while(false !== ($row = $this->DatabaseHandler->fetchArray($result)))
	{
		...
		if (!isset($classes[$class]) && file_exists(SYSCP_PATH_BASE.$file))
		{
			$this->LogHandler->info(Syscp_Handler_Log_Interface::FACILITY_SYSTEM,
			                        sprintf( 'Instanciating class %s', $class ));
			// it is not, lets load it and instanciate
			require_once SYSCP_PATH_BASE.$file;
			// instanciate
			$classes[$class] = new $class();
			// init the hook
			$classes[$class]->initialize( $this->DatabaseHandler,
			                              $this->ConfigHandler,
			                              $this->HookHandler,
			                              $this->LogHandler,
		...
		if( isset( $classes[$class] ) )
		{
			...
			$this->LogHandler->info(Syscp_Handler_Log_Interface::FACILITY_SYSTEM,
			                        sprintf('Executing %s::%s', $class, $method));
			$classes[$class]->$method( $params );

			// and remove the scheduled function call
			$this->LogHandler->info(Syscp_Handler_Log_Interface::FACILITY_SYSTEM,
			                        sprintf('Removing %s::%s from schedule list',
			                                $class, $method));
		...
		else
		{
			...
			$this->LogHandler->critical(Syscp_Handler_Log_Interface::FACILITY_SYSTEM,
			                            $error);
			$this->LogHandler->critical(Syscp_Handler_Log_Interface::FACILITY_SYSTEM,
			                            'Exiting without lockfile removal...');
