		...
		if ($this->ConfigHandler->get('env.id')!=0)
		{
			...
			if($result['id']=='')
			{
				...
				if($result['default']=='0')
				{
					...
					if( ( $result['ip']!=$this->ConfigHandler->get('system.ipaddress') ) || ( $result['ip']==$this->ConfigHandler->get('system.ipaddress') && $result2['id']!='' ) )
					{
						...
						if($result['ip']!='')
						{
							if(isset($_POST['send']) && $_POST['send']=='send')
							{
								...
								$this->HookHandler->call( 'OnDeleteIPPort',
							...
							{
								$this->TemplateHandler->showQuestion('admin_ip_reallydelete',
								                                     array('module' => 'ipsandports',
					...
					{
						$this->TemplateHandler->showError('cantdeletesystemip');
						return false;
					}
				}
				else
				{
					$this->TemplateHandler->showError('cantdeletedefaultip');
					return false;;
				}
			}
			else
			{
				$this->TemplateHandler->showError('ipstillhasdomains');
				return false;
			}
		}