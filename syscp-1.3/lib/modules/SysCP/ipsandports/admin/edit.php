		...
		if ($this->ConfigHandler->get('env.id')!=0)
		{
			...
			if($result['ip']!='')
			{
				if(isset($_POST['send']) && $_POST['send']=='send')
				{
					...
					elseif($result['ip']!=$ip && $result['ip']==$this->ConfigHandler->get('system.ipaddress') && $result3['id']=='')
					{
						$this->TemplateHandler->showError('cantchangesystemip');
						return false;
					}
					elseif($result2['id']!='' && $result2['id']!=$this->ConfigHandler->get('env.id'))
					{
					...
					else
					{
						...
						$this->HookHandler->call('OnUpdateIPPort',
