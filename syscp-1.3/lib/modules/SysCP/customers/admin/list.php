<?php
	if ($this->User['customers'] != '0')
    {
			...
			$result = $this->DatabaseHandler->query(
				...
				"WHERE ".( $this->User['customers_see_all'] ? '' : " `c`.`adminid` = '{$this->User['adminid']}' AND " )."`c`.`adminid`=`a`.`adminid` " .
			...
			if ($pages != 0)
			{
				...
				for ($count = 1; $count <= $pages+1; $count++)
				{
					...
					{
						$paging .= '<a href="' .
						           $this->createLink(array('page' => $this->ConfigHandler->get('env.page'),
						                                   'no'   => $count)) .
						           '"><b>'.$count.'</b></a>&nbsp;';
					}
					else
			...
			{
				$domains=$this->DatabaseHandler->queryFirst(
				...
				$row['deactivated']    = str_replace('0', $this->L10nHandler->get('panel.yes'), $row['deactivated']);
				$row['deactivated']    = str_replace('1', $this->L10nHandler->get('panel.no'),  $row['deactivated']);
