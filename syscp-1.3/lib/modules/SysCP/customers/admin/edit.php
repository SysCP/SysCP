               ...
               && $_POST['send'] == 'send')
            {
                ...
                else
                {
                       ...
                       && $result['standardsubdomain'] == '0')
                    {
                        ...
                        .'    `specialsettings` = \'\'';
                        $query = sprintf($query, TABLE_PANEL_DOMAINS, $stdSubdomainName, $result['customerid'], $this->User['adminid'], $stdSubdomainDocRoot, $ipandport);
                        $this->DatabaseHandler->query($query);
                        $domainid = $this->DatabaseHandler->insert_id();
                        $this->DatabaseHandler->query('UPDATE `'.TABLE_PANEL_CUSTOMERS.'` '.'SET `standardsubdomain`=\''.$domainid.'\' '.'WHERE `customerid`=\''.$result['customerid'].'\'');
                    ...
                    if($createstdsubdomain == '0'
                       && $result['standardsubdomain'] != '0')
                    {
                        $this->DatabaseHandler->query('DELETE FROM `'.TABLE_PANEL_DOMAINS.'` '.'WHERE `id`=\''.$result['standardsubdomain'].'\'');
                        $this->DatabaseHandler->query('UPDATE `'.TABLE_PANEL_CUSTOMERS.'` '.'SET `standardsubdomain`=\'0\' '.'WHERE `customerid`=\''.$result['customerid'].'\'');
                    ...
                    if($deactivated != $result['deactivated'])
                    {
                        ...
                        $this->HookHandler->call('OnDeactivateCustomer', array(
