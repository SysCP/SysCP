...
{
    if($this->User['customers_used'] < $this->User['customers']
       || $this->User['customers'] == '-1')
    {
        if(isset($_POST['send'])
           && $_POST['send'] == 'send')
        {
            ...
            $def_language = addslashes(htmlentities(html_entity_decode($_POST['def_language'])));
            ...
            {
                $this->TemplateHandler->showError('SysCP.customers.error.youcantallocatemorethanyouhave');
                return false;
            }
            ...
            if($name == '')
            {
                $this->TemplateHandler->showError(
                    ...
                    $this->L10nHandler->get('SysCP.globallang.name')
                );
                return false;
            }
            elseif($firstname == '')
            {
                $this->TemplateHandler->showError(
                    'SysCP.globallang.error.stringisempty',
                    $this->L10nHandler->get('SysCP.customers.firstname')
                );
                return false;
            }
            elseif($email == '')
            {
                $this->TemplateHandler->showError(
                    'SysCP.globallang.error.stringisempty',
                    $this->L10nHandler->get('SysCP.customers.email')
                );
                return false;
            }
            elseif(!$this->ValidationHandler->getValidator('email')->validate($email))
            {
                $this->TemplateHandler->showError('SysCP.globallang.error.emailiswrong', $email);
                return false;
            }
            else
            {
                   ...
                   && $_POST['loginname'] != '')
                {
                    ...
                    if($loginname_check['loginname'] == $loginname
                       || $loginname_check_admin['loginname'] == $loginname)
                    {
                    ...
                    {
                        $this->TemplateHandler->showError('SysCP.globallang.error.loginnameiswrong', $loginname);
                        return false;
                    }
                ...
                $result = $this->DatabaseHandler->query("INSERT INTO `".TABLE_PANEL_CUSTOMERS."` "."(`adminid`, `loginname`, `password`, `name`, `firstname`, `company`, `street`, `zipcode`, `city`, `phone`, `fax`, `email`, `customernumber`, `def_language`, `homedir`, `guid`, `diskspace`, `traffic`, `subdomains`, `emails`, `email_accounts`, `email_forwarders`, `ftps`, `mysqls`, `standardsubdomain`) "." VALUES ('{$this->User['adminid']}', '$loginname', '".md5($password)."', '$name', '$firstname', '$company', '$street', '$zipcode', '$city', '$phone', '$fax', '$email', '$customernumber','$def_language', '$documentroot', '$guid', '$diskspace', '$traffic', '$subdomains', '$emails', '$email_accounts', '$email_forwarders', '$ftps', '$mysqls', '0')");
                ...
                $this->DatabaseHandler->query("UPDATE `".TABLE_PANEL_SETTINGS."` "."SET `value`='$guid' "."WHERE `settinggroup`='system' AND `varname`='lastguid'");
                $this->DatabaseHandler->query("UPDATE `".TABLE_PANEL_SETTINGS."` "."SET `value`='$accountnumber' "."WHERE `settinggroup`='system' AND `varname`='lastaccountnumber'");
                $this->HookHandler->call('OnCreateCustomer', array(
                    'id' => $customerid,
                    'loginname' => $loginname,
                    'uid' => $guid,
                    'gid' => $guid
                ...
                $this->DatabaseHandler->query("INSERT INTO `".TABLE_PANEL_HTPASSWDS."` "."(`customerid`, `username`, `password`, `path`) "."VALUES ('$customerid', '$loginname', '$htpasswdPassword', '$path')");
                $htpasswdID = $this->DatabaseHandler->insert_id();
                $this->HookHandler->call('OnCreateHTPasswd', array(
                    'id' => $htpasswdID,
                    'path' => $path
                ));
                $result = $this->DatabaseHandler->query("INSERT INTO `".TABLE_FTP_USERS."` "."(`customerid`, `username`, `password`, `homedir`, `login_enabled`, `uid`, `gid`) "."VALUES ('$customerid', '$loginname', ENCRYPT('$password'), '$documentroot/', 'y', '$guid', '$guid')");
                $result = $this->DatabaseHandler->query("INSERT INTO `".TABLE_FTP_GROUPS."` "."(`customerid`, `groupname`, `gid`, `members`) "."VALUES ('$customerid', '$loginname', '$guid', '$loginname')");
                ...
                if($createstdsubdomain == '1')
                {
                    ...
                    $query = 'INSERT INTO `%s` '.'SET `domain`          = \'%s\', '.'    `customerid`      = \'%s\', '.'    `adminid`         = \'%s\', '.'    `parentdomainid`  = \'-1\', '.'    `documentroot`    = \'%s\', '.'    `ipandport`       = \'%s\', '.'    `zonefile`        = \'\', '.'    `isemaildomain`   = \'0\', '.'    `caneditdomain`   = \'0\', '.'    `openbasedir`     = \'1\', '.'    `safemode`        = \'1\', '.'    `access_logfile`  = \''.$accessLog.'\', '.'    `error_logfile`   = \''.$errorLog.'\', '

                    //							       . '    `speciallogfile`  = \'0\', '

                    .'    `specialsettings` = \'\'';
                    $query = sprintf($query, TABLE_PANEL_DOMAINS, $stdSubdomainName, $customerid, $this->User['adminid'], $stdSubdomainDocRoot, $ipandport);
                    $this->DatabaseHandler->query($query);
                    $domainid = $this->DatabaseHandler->insert_id();
                    $this->DatabaseHandler->query('UPDATE `'.TABLE_PANEL_CUSTOMERS.'` '.'SET `standardsubdomain`=\''.$domainid.'\' '.'WHERE `customerid`=\''.$customerid.'\'');
                ...
                if($sendpassword == '1')
                {
                    ...
                    $mail_subject = html_entity_decode(replace_variables((($result['value'] != '') ? $result['value'] : $this->L10nHandler->get('SysCP.customers.createcustomer_subj')), $replace_arr));
                    ...
                    $mail_body = html_entity_decode(replace_variables((($result['value'] != '') ? $result['value'] : $this->L10nHandler->get('SysCP.customers.createcustomer_body')), $replace_arr));
                    mail("$firstname $name <$email>", $mail_subject, $mail_body, "From: {$this->User['name']} <{$this->User['email']}>\r\n");
                }

                # Plugins logic support

                Syscp::uses('Syscp.Handler.Plugins');
                $syscp_plugins_logic = new Syscp_Handler_Plugins();
                $syscp_plugins_logic->initialize(array(
                ...
                $syscp_plugins_logic->getPluginsLogic($this->vendor, $this->module, $this->area, $this->action);
        ...
        else
        {
            ...
            # Plugins display support

            Syscp::uses('Syscp.Handler.Plugins');
            $syscp_plugins_display = new Syscp_Handler_Plugins();
            $syscp_plugins_display->initialize(array(
                'moduleconfig' => $this->moduleConfig,
                'theme' => $this->theme,
                'language' => $this->language,
                'confighandler' => $this->ConfigHandler,
            ...
            $syscp_plugins_display->getPluginsDisplay($this->vendor, $this->module, $this->area, $this->action);
