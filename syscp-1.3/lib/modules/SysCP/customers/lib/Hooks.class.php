...
class Syscp_Customers_Hooks extends Syscp_BaseHook
{
    ...
    public function OnCalcTrafficAtCron($params = array())
    {
        ...
        {
            $query = 'SELECT SUM(`http`) AS `http_sum`, '.'       SUM(`mail`) AS `mail_sum`, '.'       SUM(`ftp_up`) AS `ftp_up_sum`, '.'       SUM(`ftp_down`) AS `ftp_down_sum` '.'FROM `'.TABLE_PANEL_TRAFFIC.'` '.'WHERE `customerid` = '.(int)$customer['customerid'].' '.'  AND `month` = '.(int)$today['m'].' '.'  AND `year`  = '.(int)$today['y'];
            $traffic = $db->queryFirst($query);
            $traffic = $traffic['http_sum']+$traffic['mail_sum']+$traffic['ftp_up_sum']+$traffic['ftp_down_sum'];

            // And now we put the values into the customers account row

            $query = 'UPDATE `'.TABLE_PANEL_CUSTOMERS.'` '.'SET `traffic_used` = '.$traffic.' '.'WHERE `customerid` = '.(int)$customer['customerid'];
            $db->query($query);
        }
    }
    ...
    public function OnCalcDiskusageAtCron($params = array())
    {
        ...
        $dsn = sprintf('mysql://%s:%s@%s', $config->get('sql.root.user'), $config->get('sql.root.password'), $config->get('sql.host'));
        ...
        while(false !== ($customer = $db->fetchArray($result)))
        {
            ...
            $query = 'SELECT * '.'FROM `'.TABLE_PANEL_DOMAINS.'` '.'WHERE `customerid` = '.(int)$customer['customerid'];
            ...
            $query = 'UPDATE `'.TABLE_PANEL_CUSTOMERS.'` '.'SET `diskspace_used` = '.(int)$diskspace.' '.'WHERE `customerid` = '.(int)$customer['customerid'];
            $db->query($query);
        }
    }
}
