...
if($this->ConfigHandler->get('env.id') != 0)
{
    ...
    if(isset($result['username'])
       && $result['username'] != $this->User['loginname'])
    {
        if(isset($_POST['send'])
           && $_POST['send'] == 'send')
        {
        ...
        {
            $this->TemplateHandler->showQuestion('SysCP.ftp.question.reallydelete', array(
                'module' => 'ftp',
    ...
    {
        $this->TemplateHandler->showError('SysCP.ftp.error.cantdeletemainaccount');
        return false;
    }
}
