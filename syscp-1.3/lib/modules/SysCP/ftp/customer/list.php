...
if($pages != 0)
{
    ...
    $result = $this->DatabaseHandler->query("SELECT `id`, `username`, `homedir` FROM `".TABLE_FTP_USERS."` WHERE `customerid`='".$this->User['customerid']."' ORDER BY `username` ASC "." LIMIT $pagestart , ".$this->ConfigHandler->get('panel.paging').";");
    $paging = '';
