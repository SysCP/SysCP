...
class Syscp_ProFTP_Hooks extends Syscp_BaseHook
{
	...
	public function OnCalcTrafficAtCron($params = array())
	{
		...
		$isChecked = array(); // this is an array of customerid's which have already
		...
		while (false !== ($row = $db->fetchArray($result)))
		{
			...
			if(!isset($isChecked[$row['customerid']]))
			{
				...
				else
				{
					$query   = 'INSERT INTO `'.TABLE_PANEL_TRAFFIC.'` '
					         . 'SET `customerid` = '.(int)$row['customerid'].', '
					         . '    `day`   = '.(int)$today['d'].', '
					         . '    `month` = '.(int)$today['m'].', '
					         . '    `year`  = '.(int)$today['y'].' ';
					$db->query($query);
			...
			$query   = 'UPDATE `'.TABLE_PANEL_TRAFFIC.'` '
			         . 'SET `ftp_up`   = `ftp_up`   + '.$row['up_bytes'].', '
			         . '    `ftp_down` = `ftp_down` + '.$row['down_bytes'].' '
			         . 'WHERE `customerid` = '.(int)$row['customerid'].' '
			         . '  AND `day`   = '.(int)$today['d'].' '
			         . '  AND `month` = '.(int)$today['m'].' '
			         . '  AND `year`  = '.(int)$today['y'].' ';
