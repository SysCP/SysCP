   ...
   && $_POST['send'] == 'send')
{
    ...
    // string: the table where the userrecord can be found

    $uidField = '';

    // string: name of uid field in the table
    ...
    $curLanguage = addslashes(htmlentities(html_entity_decode($_POST['language'])));
    $curTheme = addslashes(htmlentities(html_entity_decode($_POST['theme'])));
    ...
    // our passes are md5 encrypted, lets encrypt the form pass
    ...
    $row = $this->DatabaseHandler->queryFirst($query);
    ...
    else
    {
        ...
        else
        {
            ...
            $this->LogHandler->warning(Syscp_Handler_Log_Interface::FACILITY_AUTH, sprintf('Login failed: User %s unkown from %s', $loginname, $this->ConfigHandler->get('env.remote_addr')));
            $this->TemplateHandler->showError('SysCP.login.error.login');
            return false;
        }
    }
       ...
       && $row['lastlogin_fail'] > (time()-$this->ConfigHandler->get('login.deactivatetime')))
    {
        ...
        $this->LogHandler->warning(Syscp_Handler_Log_Interface::FACILITY_AUTH, sprintf('User %s tried to login but is blocked.', $row['loginname']));
        $this->TemplateHandler->showError('SysCP.login.error.blocked', $this->ConfigHandler->get('login.deactivatetime'));
        return false;
    }
    ...
    if($row['password'] == $password)
    {
        ...
        $query = 'UPDATE `%s` '.'SET `lastlogin_succ`  = \'%s\', '.'    `loginfail_count` = \'0\' '.'WHERE `%s` = \'%s\' ';
    ...
    else
    {
        ...
        $query = 'UPDATE `%s` '.'SET `lastlogin_fail`  = \'%s\', '.'    `loginfail_count` = `loginfail_count` + 1 '.'WHERE `%s` = \'%s\'';
        $query = sprintf($query, $userTable, time(), $uidField, $row[$uidField]);
        $this->DatabaseHandler->query($query);
        $this->LogHandler->warning(Syscp_Handler_Log_Interface::FACILITY_AUTH, sprintf('User %s wrong password from %s', $row['loginname'], $this->ConfigHandler->get('env.remote_addr')));
    }
       ...
       && $row['userid'] != '')
    {
        ...
        $this->LogHandler->info(Syscp_Handler_Log_Interface::FACILITY_AUTH, sprintf('User %s logged in successfully from %s', $row['loginname'], $this->ConfigHandler->get('env.remote_addr')));

        // and create the new logfile instance for this user

        $this->LogHandler->setUsername($row['loginname']);
        $this->LogHandler->info(Syscp_Handler_Log_Interface::FACILITY_USER, sprintf('User %s logged in successfully from %s', $row['loginname'], $this->ConfigHandler->get('env.remote_addr')));
...
else
{
    ...
    $languageList['profile'] = $this->L10nHandler->get('SysCP.login.profile_lng');
