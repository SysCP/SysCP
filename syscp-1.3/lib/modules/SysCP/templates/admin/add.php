<?php
		...
		elseif(isset($_GET['subjectid']))
		{
			...
			{
				$lang = addslashes ( htmlentities ( _html_entity_decode ( $_POST['language'] ) ) ) ;
				...
				{
					$template_options[$tpl] = $this->L10nHandler->get('admin.templates.'.$tpl);
				}
	...
	//				$template_options.=makeoption($lng['admin']['templates'][$template],$template);
		//		}
			...
			else if(isset($_POST['send']) && $_POST['send']=='send')
			{
				...
				if($language == '')
				{
					$this->TemplateHandler->showError('nolanguageselect');
					return false;
				}
				elseif($subject == '')
				{
				...
				{
					$this->TemplateHandler->showError('templatenotfound');
					return false;
				}
			...
			else
			{
				...
				{
					$this->TemplateHandler->showError('alltemplatesdefined');
					return false;
				}
			}
