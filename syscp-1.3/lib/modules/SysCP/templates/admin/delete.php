   ...
   && $mailbodyid != 0)
{
    ...
    if($result['varname'] != '')
    {
        ...
        {
            $this->TemplateHandler->showQuestion('admin_template_reallydelete', array(
                'module' => 'templates',
                'subjectid' => $subjectid,
                'mailbodyid' => $mailbodyid,
                'action' => $this->ConfigHandler->get('env.action')
            ), $result['language'].' - '.$this->L10nHandler->get('SysCP.templates.'.str_replace('_subject', '', $result['varname'])));
        }
    }
}
