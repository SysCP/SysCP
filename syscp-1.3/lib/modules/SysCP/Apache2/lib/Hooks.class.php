    ...
    public function OnCalcTrafficAtCron($params = array())
    {
        ...
        while(false !== ($customer = $db->fetchArray($result)))
        {
            ...
            if(Syscp::isReadableFile($logFile))
            {
                ...
                $query = 'SELECT * '.'FROM `'.TABLE_PANEL_TRAFFIC.'` '.'WHERE `customerid` = '.(int)$customer['customerid'].' '.'  AND `day`   = '.(int)$today['d'].' '.'  AND `month` = '.(int)$today['m'].' '.'  AND `year`  = '.(int)$today['y'].' ';
                ...
                if(isset($oldTraffic['http']))
                {
                    ...
                    $query = 'UPDATE `'.TABLE_PANEL_TRAFFIC.'` '.'SET `http` = '.(int)$traffic.' '.'WHERE `customerid` = '.(int)$customer['customerid'].' '.'  AND `day`   = '.(int)$today['d'].' '.'  AND `month` = '.(int)$today['m'].' '.'  AND `year`  = '.(int)$today['y'].' ';
                }
                else
                {
                    $query = 'INSERT INTO `'.TABLE_PANEL_TRAFFIC.'` '.'SET `http` = '.(int)$traffic.', '.'    `customerid` = '.(int)$customer['customerid'].', '.'    `day`   = '.(int)$today['d'].', '.'    `month` = '.(int)$today['m'].', '.'    `year`  = '.(int)$today['y'].' ';
                }
