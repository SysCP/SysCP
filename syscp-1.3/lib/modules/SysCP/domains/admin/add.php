...
if($this->User['domains_used'] < $this->User['domains']
   || $this->User['domains'] == '-1')
{
    if(isset($_POST['send'])
       && $_POST['send'] == 'send')
    {
        ...
        $domain = $this->IdnaHandler->encode(preg_replace(array(
            '/\:(\d)+$/',
            '/^https?\:\/\//'
        ...
        $this->ValidationHandler->addValidator('domain', new Syscp_Validator_Regex('/^([a-z0-9][a-z0-9\-]+\.)+[a-z]{2,4}$/i'));
        ...
        elseif(!$this->ValidationHandler->getValidator('domain')->validate($domain))
        {
            $this->TemplateHandler->showError(
                'SysCP.globallang.error.stringiswrong',
                'mydomain'
            );
            return false;
        }
        ...
        elseif($customerid == 0)
        {
            $this->TemplateHandler->showError('SysCP.domains.error.adduserfirst');
            return false;
        }
        elseif($domain_check['domain'] == $domain)
        {
            $this->TemplateHandler->showError('SysCP.domains.error.domainalreadyexists', $domain);
            return false;
        }
        ...
        else
        {
            ...
            {
                $this->TemplateHandler->showQuestion('SysCP.domains.question.reallydisablesecuritysetting', array(
                    'module' => 'domains',
            ...
            $this->ValidationHandler->removeValidator('domain');
               ...
               && (!isset($_POST['reallydocroot']) || $_POST['reallydocroot'] != 'reallydocroot'))
            {
                ...
                $this->TemplateHandler->showQuestion('SysCP.domains.question.reallydocrootoutofcustomerroot', $params);
            ...
            $query = 'INSERT INTO `'.TABLE_PANEL_DOMAINS.'` '.'SET `domain`           = \''.$domain.'\', '.'    `customerid`       = \''.$customerid.'\', '.'    `adminid`          = \''.$this->User['adminid'].'\', '.'    `documentroot`     = \''.$documentroot.'\', '.'    `ipandport`        = \''.$ipandport.'\', '.'    `aliasdomain`      = '.$aliasdomain.', '.'    `zonefile`         = \''.$zonefile.'\', '.'    `isbinddomain`     = \''.$isbinddomain.'\', '.'    `isemaildomain`    = \''.$isemaildomain.'\', '.'    `subcanemaildomain`= \''.$subcanemaildomain.'\', '.'    `caneditdomain`    = \''.$caneditdomain.'\', '.'    `openbasedir`      = \''.$openbasedir.'\', '.'    `safemode`         = \''.$safemode.'\', '.'    `access_logfile`   = \''.$accessLog.'\', '.'    `error_logfile`    = \''.$errorLog.'\', '
            ...
            $query = 'UPDATE `'.TABLE_PANEL_ADMINS.'` '.'SET `domains_used` = `domains_used` + 1 '.'WHERE `adminid` = \''.$this->User['adminid'].'\'';
            $this->DatabaseHandler->query($query);
    ...
    else
    {
        ...
        $customers = array();
        $query = 'SELECT `customerid`, `loginname`, `name`, `firstname` '.'FROM `'.TABLE_PANEL_CUSTOMERS.'` ';
        ...
        {
            $customers[$row_customer['customerid']] = $row_customer['name'].' '.$row_customer['firstname'].' ('.$row_customer['loginname'].')';
        }

        $this->TemplateHandler->set('customers', $customers);
        $ipsandports = array();
        $query = 'SELECT `id`, `ip`, `port` '.'FROM `'.TABLE_PANEL_IPSANDPORTS.'` '.'ORDER BY `ip` ASC';
        ...
        $query = 'SELECT `id` '.'FROM `'.TABLE_PANEL_DOMAINS.'` `d`, `'.TABLE_PANEL_CUSTOMERS.'` `c` '.'WHERE `d`.`id` = `c`.`standardsubdomain`';
