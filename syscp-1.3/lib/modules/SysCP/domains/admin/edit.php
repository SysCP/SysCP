...
if($this->ConfigHandler->get('env.id') != 0)
{
    ...
    $result = $this->DatabaseHandler->queryFirst("SELECT `d`.`id`, `d`.`domain`, `d`.`customerid`, `d`.`documentroot`, `d`.`ipandport`, `d`.`aliasdomain`, `d`.`isbinddomain`, `d`.`isemaildomain`, `d`.`subcanemaildomain`, `d`.`caneditdomain`, `d`.`zonefile`, `d`.`openbasedir`, `d`.`safemode`, `d`.`speciallogfile`, `d`.`specialsettings`, `d`.`access_logfile`, `d`.`error_logfile`, `c`.`loginname`, `c`.`name`, `c`.`firstname` "."FROM `".TABLE_PANEL_DOMAINS."` `d` "."LEFT JOIN `".TABLE_PANEL_CUSTOMERS."` `c` USING(`customerid`) "."WHERE `d`.`parentdomainid`='0' AND `d`.`id`='".$this->ConfigHandler->get('env.id')."'".($this->User['customers_see_all'] ? '' : " AND `d`.`adminid` = '{$this->User['adminid']}' "));
    ...
    if($result['domain'] != '')
    {
           ...
           && $_POST['send'] == 'send')
        {
            ...
            {
                $this->throwError('SysCP.domains.error.domainisaliasorothercustomer');
            }

            if(($openbasedir == '0' || $safemode == '0')
               && (!isset($_POST['reallydoit']) || $_POST['reallydoit'] != 'reallydoit')
               && $this->User['change_serversettings'] == '1')
            {
