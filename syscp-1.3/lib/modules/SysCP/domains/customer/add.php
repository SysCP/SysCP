<?php
			if($this->User['subdomains_used'] < $this->User['subdomains'] || $this->User['subdomains'] == '-1')
			{
				if(isset($_POST['send']) && $_POST['send']=='send')
				{
					$subdomain    = $this->IdnaHandler->encode(preg_replace(Array('/\:(\d)+$/','/^https?\:\/\//'),'',addslashes($_POST['subdomain'])));
...
//						$path = $this->User['homedir'].$path;
					...
					if($path=='')
					{
						$this->TemplateHandler->showError('patherror');
						return false;
					}
					elseif($subdomain=='')
					{
						$this->TemplateHandler->showError(array('stringisempty','domainname'));
						return false;
					}
					elseif($subdomain=='www')
					{
						$this->TemplateHandler->showError('wwwnotallowed');
						return false;
					}
					elseif(preg_match('/.*\..*/',$subdomain))
					{
						$this->TemplateHandler->showError('subdomainiswrong',$subdomain);
						return false;
					}
					elseif($domain=='')
					{
						$this->TemplateHandler->showError('domaincantbeempty');
						return false;
					}
					elseif($completedomain_check['domain']==$completedomain)
					{
						$this->TemplateHandler->showError('domainexistalready',$completedomain);
						return false;
					}
					elseif($domain_check['domain']!=$domain)
					{
						$this->TemplateHandler->showError('maindomainnonexist',$domain);
						return false;
					}
				...
				else
				{
					...
					$pathSelect = makePathfield( $this->User['homedir'], $this->User['guid'],
					                             $this->User['guid'], $this->ConfigHandler->get('panel.pathedit') );
