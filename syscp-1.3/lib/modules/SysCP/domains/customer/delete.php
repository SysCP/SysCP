<?php
			...
			if(isset($result['parentdomainid']) && $result['parentdomainid']!='0' && $alias_check['count'] == 0)
			{
				if(isset($_POST['send']) && $_POST['send']=='send')
				{
					if($result['isemaildomain'] == '1')
					{
						...
						if($emails['count'] != '0')
						{
							$this->TemplateHandler->showError('domains_cantdeletedomainwithemail');
							return false;
						}
					}
				...
				{
					$this->TemplateHandler->showQuestion('domains_reallydelete',
			...
			{
				$this->TemplateHandler->showError('domains_cantdeletemaindomain');
				return false;
			}