	...
	function reverseDomain($domain)
	{
		...
		return $result;
			...
			$result = $this->DatabaseHandler->query(
				...
				"       `pd`.`domain` AS `parentdomain` " .
				"FROM `".TABLE_PANEL_DOMAINS."` `d` " .
				"LEFT JOIN `".TABLE_PANEL_DOMAINS."` `ad` ON `d`.`aliasdomain`=`ad`.`id` " .
				"LEFT JOIN `".TABLE_PANEL_DOMAINS."` `pd` ON `d`.`parentdomainid`=`pd`.`id` " .
				"WHERE `d`.`customerid`='".$this->User['customerid']."' " .
				"  AND `d`.`id` <> ".$this->User['standardsubdomain']
			);
			$domainList        = array();
