...
if($this->User['change_serversettings'] == '1')
{
       ...
       && $_POST['send'] == 'send')
    {
        ...
        if($_POST['customer_accountprefix'] != $this->ConfigHandler->get('customer.accountprefix'))
        {
            ...
            $this->ValidationHandler->addValidator('username_prefix', '/^[a-zA-Z0-9][a-zA-Z0-9\-\_]*$/');

            if($this->ValidationHandler->getValidator('username_prefix')->validate($value))
            {
            ...
            $this->ValidationHandler->removeValidator('username_prefix');
        }

        if($_POST['customer_mysqlprefix'] != $this->ConfigHandler->get('customer.mysqlprefix'))
        {
            $value = addslashes($_POST['customer_mysqlprefix']);
            $this->ValidationHandler->addValidator('mysql_prefix', '/^[a-zA-Z0-9\-\_]+$/');

            if($this->ValidationHandler->getValidator('mysql_prefix')->validate($value))
            {
            ...
            $this->ValidationHandler->removeValidator('mysql_prefix');
        }
        ...
        if($_POST['system_ipaddress'] != $this->ConfigHandler->get('system.ipaddress'))
        {
            ...
            if($this->ConfigHandler->get('sql.host') != 'localhost')
            {
                ...
                $update_users = '"'.$this->ConfigHandler->get('sql.user').'", '.'"'.$this->ConfigHandler->get('sql.root.user').'"';
        ...
        {
            $value = addslashes(htmlentities(html_entity_decode($_POST['panel_standardlanguage'])));
        ...
        if($_POST['panel_phpmyadmin_url'] != $this->ConfigHandler->get('panel.phpmyadmin_url'))
        {
            ...
            if($this->ConfigHandler->get('panel.phpmyadmin_url') != '')
            {
                ...
                else
                {
                    ...
                    $query = 'UPDATE `'.TABLE_PANEL_NAVIGATION.'` '.'SET `url`="'.$value.'" '.'WHERE `lang` = "menue;mysql;phpmyadmin"';
                }
            }
            else
            {
                ...
                $query = 'INSERT INTO `'.TABLE_PANEL_NAVIGATION.'` '.'SET `lang`       = "menue;mysql;phpmyadmin", '.'    `url`        = "'.$value.'", '.'    `order`      = "'.$new.'", '.'    `area`       = "customer", '.'    `new_window` = "1", '.'    `required_resources` = "mysqls_used", '.'    `parent_url` = "customer_mysql.php"';
            }
        ...
        if($_POST['panel_webmail_url'] != $this->ConfigHandler->get('panel.webmail_url'))
        {
            ...
            if($this->ConfigHandler->get('panel.webmail_url') != '')
            {
                ...
                else
                {
                    ...
                    $query = 'UPDATE `'.TABLE_PANEL_NAVIGATION.'` '.'SET `url`="'.$value.'" '.'WHERE `lang` = "menue;email;webmail"';
                }
            }
            else
            {
                ...
                $max = $this->DatabaseHandler->queryFirst($query);
                $new = floor($max['max']/10)+10;
                $query = 'INSERT INTO `'.TABLE_PANEL_NAVIGATION.'` '.'SET `lang`       = "menue;email;webmail", '.'    `url`        = "'.$value.'", '.'    `order`      = "'.$new.'", '.'    `area`       = "customer", '.'    `new_window` = "1", '.'    `required_resources` = "emails_used", '.'    `parent_url` = "customer_email.php"';
            }
        ...
        if($_POST['panel_webftp_url'] != $this->ConfigHandler->get('panel.webftp_url'))
        {
            ...
            if($this->ConfigHandler->get('panel.webftp_url') != '')
            {
                ...
                else
                {
                    ...
                    $query = 'UPDATE `'.TABLE_PANEL_NAVIGATION.'` '.'SET `url`="'.$value.'" '.'WHERE `lang` = "menue;ftp;webftp"';
                }
            }
            else
            {
                ...
                $max = $this->DatabaseHandler->queryFirst($query);
                $new = floor($max['max']/10)+10;
                $query = 'INSERT INTO `'.TABLE_PANEL_NAVIGATION.'` '.'SET `lang`       = "menue;ftp;webftp", '.'    `url`        = "'.$value.'", '.'    `order`      = "'.$new.'", '.'    `area`       = "customer", '.'    `new_window` = "1", '.'    `parent_url` = "customer_ftp.php"';
            }
