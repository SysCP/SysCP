<?php
			if( (isset($_POST['send'])) && ($_POST['send']=='send') )
			{
				...
				$path_dupe_check = $this->DatabaseHandler->query_first("SELECT `id`, `path` FROM `".TABLE_PANEL_HTACCESS."` WHERE `path`='$path' AND `customerid`='".$this->User['customerid']."'");
					if(!$_POST['path'])
					{
						$this->TemplateHandler->showError('invalidpath');
						return false;
					}
					...
					{
						$this->TemplateHandler->showError('mustbeurl');
						return false;
					}
...
//					}
					...
					{
						$this->TemplateHandler->showError('errordocpathdupe',$userpath);
						return false;
					}
					elseif ($path == '')
					{
...
//						'        "'.$error401path.'", ' .
					...
					$this->HookHandler->call( 'OnCreateHTAccess',
					              array( 'id'   => $htaccessID,
					                     'path' => $path ) );
			...
			else
			{
				...
				$options_indexes = array( 1 => $this->L10nHandler->get('panel.yes'),
