...
while($row = $this->DatabaseHandler->fetch_array($result))
{
    ...
    $row['options_indexes'] = str_replace('1', $this->L10nHandler->get('SysCP.globallang.yes'), $row['options_indexes']);
    $row['options_indexes'] = str_replace('0', $this->L10nHandler->get('SysCP.globallang.no'), $row['options_indexes']);
    $htaccess[] = $row;
}
