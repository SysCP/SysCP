 ...
 */
	...
	if( $settings['panel']['version'] == '1.2.13' )
	{
		$query = 
 		...
 		$query = 
 			...
 			'CHANGE `type` `file` VARCHAR( 255 ) NOT NULL , ' .
 			'CHANGE `data` `class` VARCHAR( 255 ) NOT NULL ';
 		$db->query($query);

		$query = 
			'ALTER TABLE `panel_tasks` ' .
			'ADD `method` VARCHAR( 255 ) NOT NULL , ' .
		...
		$queryTpl = 
		    ...
		    '    `class`    = \'%s\', ' .
		    '    `method`   = \'%s\' ';
		$db->query( sprintf( $queryTpl, 
		                     TABLE_PANEL_HOOKS, 
		                     0,
		                     'core', 
		                     'deleteCustomer',
		                     'lib/classes/Org/Syscp/Core/Hooks/Apache.class.php',
		                     'Org_Syscp_Core_Hooks_Apache', 
		                     'deleteCustomer' ) );
		$db->query( sprintf( $queryTpl, 
		                     TABLE_PANEL_HOOKS, 
		                     0,
		...
		$db->query( sprintf( $queryTpl, 
		                     ...
		                     'core', 
		                     'deleteCustomer',
		                     'lib/classes/Org/Syscp/Core/Hooks/Bind.class.php',
		                     'Org_Syscp_Core_Hooks_Bind', 
		                     'deleteCustomer' ) );
		$db->query( sprintf( $queryTpl, 
		                     TABLE_PANEL_HOOKS, 
		                     0,
		...
		$db->query( sprintf( $queryTpl, 
		                     ...
		                     'core', 
		                     'createCustomer',
		                     'lib/classes/Org/Syscp/Core/Hooks/Customer.class.php',
		                     'Org_Syscp_Core_Hooks_Customer', 
		                     'createCustomer' ) );
		// Update Version
		$query = 
			'UPDATE `%s` ' .
			'SET `value` = \'1.2.13-martin.1\' ' .
			'WHERE `settinggroup` = \'panel\' ' .
			'AND `varname` = \'version\'';
		$query = sprintf( $query, TABLE_PANEL_SETTINGS);
		$db->query($query);
	...
	if( $settings['panel']['version'] == '1.2.13-martin.1' )
	{
		...
		$query = sprintf( $query, TABLE_PANEL_TASKS, 
		                          'lib/classes/Org/Syscp/Core/Hooks/Traffic.class.php',
		                          'Org_Syscp_Core_Hooks_Traffic',
		                          'cronCalcTraffic',
