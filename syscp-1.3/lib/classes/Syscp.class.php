...
final class Syscp
{
	 ...
	 * @var array
	 */
	static private $USES_EXTENSIONS = array('.php', '.class.php');
	 ...
	 *
	 * Basically this method only replaces only the dots of the namespace with
	...
	static public function uses($namespace, $required = true)
	{
		...
		{
			if(self::isReadableFile($file.$ext) && !$isLoaded)
			{
	...
	/**
	 * This method loads the given config file parses it and return an array
	...
	/**
	 * Convenience method which does a file_exists and is_readable check on the given filename
	 *
	 * @author Martin Burchert <eremit@adm1n.de>
	 *
	 * @param  string  $filename Path and Filename of the file to be checked
	 *
	...
	static public function exec($command)
	{
		...
		// continue with commandexec_string, $result);
		exec($command, $result);
		return $result;
	}
}