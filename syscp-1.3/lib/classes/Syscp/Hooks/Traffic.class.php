...
class Syscp_Hooks_Traffic extends Syscp_BaseHook
{
    ...
    public function OnCalcTrafficAtCron($params = array())
    {
        ...
        $log->info(Syscp_Handler_Log_Interface::FACILITY_USER, '-- cronCalcTraffic: Checking if we need to calculate traffic and diskspace...');

        // check if there has already been one traffic run today
        //if($config->get('system.last_traffic_run') != date('dmy'))
        //{

        $log->info(Syscp_Handler_Log_Interface::FACILITY_USER, '-- cronCalcTraffic: Starting traffic accounting');
        ...
        while(false !== ($row = $db->fetchArray($result)))
        {
            ...
            $log->info(Syscp_Handler_Log_Interface::FACILITY_USER, sprintf('-- cronCalcTraffic: counting http traffic for %s', $row['loginname']));
            ...
            $log->info(Syscp_Handler_Log_Interface::FACILITY_USER, sprintf('-- cronCalcTraffic: counting ftp traffic for %s', $row['loginname']));
            ...
            $query = 'SELECT SUM(`up_bytes`)   AS `up_bytes_sum`, '.'       SUM(`down_bytes`) AS `down_bytes_sum` '.'FROM `%s` '.'WHERE `customerid` = \'%s\'';
            ...
            {
                $log->info(Syscp_Handler_Log_Interface::FACILITY_USER, sprintf('--cronCalcTraffic: creating traffic sum for %s', $row['loginname']));

                // query the old traffic the user currently has

                $query = 'SELECT SUM(`http`)     AS `http_sum`, '.'       SUM(`ftp_up`)   AS `ftp_up_sum`, '.'       SUM(`ftp_down`) AS `ftp_down_sum`, '.'       SUM(`mail`)     AS `mail_sum` '.'FROM `%s` '.'WHERE `year`       = \'%s\' '.'  AND `month`      = \'%s\' '.'  AND `day`        < \'%s\' '.'  AND `customerid` = \'%s\' ';
                $query = sprintf($query, TABLE_PANEL_TRAFFIC, date('Y', $yesterday), date('m', $yesterday), date('d', $yesterday), $row['customerid']);
            ...
            {
                $log->info(Syscp_Handler_Log_Interface::FACILITY_USER, sprintf('--cronCalcTraffic: creating traffic (new month) sum for %s', $row['loginname']));
            ...
            $query = 'REPLACE INTO `%s` '.'      ( `customerid`, `year`,   `month`,    `day`, '.'               `ftp_up`, `ftp_down`, `mail`) '.'VALUES( \'%s\',       \'%s\',   \'%s\',     \'%s\', '.'              \'%s\',   \'%s\',     \'%s\')';
            $query = sprintf($query, TABLE_PANEL_TRAFFIC, $row['customerid'], date('Y', $yesterday), date('m', $yesterday), date('d', $yesterday), $new['ftp_up'], $new['ftp_down'], $new['mail']);
            $db->query($query);
            ...
            if(date('d') == '01')
            {
                $query = 'UPDATE `%s` '.'SET `up_bytes`   = \'0\', '.'    `down_bytes` = \'0\'  '.'WHERE `customerid` = \'%s\'';
                $query = sprintf($query, TABLE_FTP_USERS, $row['customerid']);
                $db->query($query);
            }
            ...
            $log->info(Syscp_Handler_Log_Interface::FACILITY_USER, sprintf('-- cronCalcTraffic: counting diskspace for webs of %s', $row['loginname']));
            ...
            $log->info(Syscp_Handler_Log_Interface::FACILITY_USER, sprintf('-- cronCalcTraffic: counting diskspace for mails of %s', $row['loginname']));
            ...
            $log->info(Syscp_Handler_Log_Interface::FACILITY_USER, sprintf('-- cronCalcTraffic: counting diskspace for databases of %s', $row['loginname']));
            ...
            $query = 'SELECT `databasename` '.'FROM `%s` '.'WHERE `customerid` = \'%s\'';
            ...
            $query = 'UPDATE `%s` '.'SET `diskspace_used` = \'%s\', '.'    `traffic_used`   = \'%s\' '.'WHERE `customerid` = \'%s\' ';
            $query = sprintf($query, TABLE_PANEL_CUSTOMERS, $diskusage, $new['all'], $row['customerid']);
            $db->query($query);
        }
        ...
        while(true === ($row = $db->fetch_array($result)))
        {
            ...
            if(isset($admin_traffic[$row['adminid']]))
            {
                ...
                $query = 'REPLACE INTO `%s` '.'       ( `adminid`, `year`, `month`, `day`, '.'         `http`, `ftp_up`, `ftp_down`, `mail`) '.'VALUES ( \'%s\', \'%s\', \'%s\', \'%s\', '.'         \'%s\', \'%s\', \'%s\', \'%s\' )';
                $query = sprintf($query, TABLE_PANEL_TRAFFIC_ADMINS, $row['adminid'], date('Y', $yesterday), date('m', $yesterday), date('d', $yesterday), $admin_traffic[$row['adminid']]['http'], $admin_traffic[$row['adminid']]['ftp_up'], $admin_traffic[$row['adminid']]['ftp_down'], $admin_traffic[$row['adminid']]['mail']);
                $db->query($query);

                // additionally put the results in the admin/reseller user record

                $query = 'UPDATE `%s` '.'SET `traffic_used` = \'%s\' '.'WHERE `adminid` = \'%s\' ';
                $query = sprintf($query, TABLE_PANEL_ADMINS, $admin_traffic[$row['adminid']]['all'], $row['adminid']);
                $db->query($query);
            }
        }
        ...
        $query = 'UPDATE `%s` '.'SET `value` = \'%s\' '.'WHERE `settinggroup` = \'system\' '.'  AND `varname`      = \'last_traffic_run\' ';
