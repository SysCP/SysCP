...
class Syscp_Hooks_Bind extends Syscp_BaseHook 
{
	...
	function cronRebuildBindConfig( $params = array() )
	{
		...
		$log->info( '-- cronRebuildBindConfig: Recreating bind config file' );
		...
		$log->info( '-- cronRebuildBindConfig: restarting bind...' );
		safe_exec($config->get('system.bindreload_command') );
	}
}
