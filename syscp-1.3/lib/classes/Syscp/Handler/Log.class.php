...
class Syscp_Handler_Log implements Syscp_Handler_Log_Interface
{
	...
	public function initialize($params = array())
	{
		// Lets do some param checks first
			// the 'logdir'
		if(isset($params['logdir']) && trim((string)$params['logdir'])!='')
		{
		...
		$this->_instances[self::FACILITY_AUTH]   =
			&Log::factory('file', $this->_logdir.'auth.log', 'Auth', array());
		$this->_instances[self::FACILITY_USER]   = null;
		$this->_instances[self::FACILITY_SYSTEM] =
			&Log::factory('file', $this->_logdir.'system.log', 'System', array());
		$this->_instances[self::FACILITY_DEBUG] =
			&Log::factory('file', $this->_logdir.'debug.log', 'Debug', array());
	}

	public function critical($facility, $message)
	{
		if(isset($this->_instances[$facility]) && !is_null($this->_instances[$facility]))
		{
	...
	public function setUsername($username)
	{
		...
		$this->_instances[self::FACILITY_USER] =
			&Log::factory( 'file', $this->_logdir.'user-'.$username.'.log', $username);
	}
}
