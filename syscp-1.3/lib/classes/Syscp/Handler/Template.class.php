...
class Syscp_Handler_Template implements Syscp_Handler_Template_Interface
{
	...
	public function initialize($params = array())
	{
		...
		$required = array('Controller'   => 'Syscp_FrontController_Interface',
		...
		$this->Smarty->register_function('l10n', array($this, 'smarty_function_l10n'));
		$this->Smarty->register_function('url',  array($this, 'smarty_function_url'));
	...
	public function useTheme($theme)
	{
		...
		{
			$erro = sprintf(self::ERROR_COMPILE_DIR, $compileDir);
	...
	public function showError($error, $replacer = '')
	{
		...
		if ($this->L10nHandler->exists('error.'.$error))
		{
			$error = $this->L10nHandler->get('error.'.$error);
		}
		else
		{
			$error = 'Unknown Error';
	...
	public function showQuestion($question, $params = array(), $replacer = '', $targetUrl = '')
	{
		...
		$question = $this->L10nHandler->get('question.'.$question);
