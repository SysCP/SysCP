...
class Syscp_Handler_Hook implements Syscp_Handler_Hook_Interface
{
    ...
    private function loadHooklist()
    {
        ...
        {
            $this->addHook($row['hook'], $row['priority'], $row['file'], $row['class'], $row['method']);
        }
    }
    ...
    public function call($hookName, $params = array())
    {
        ...
        if(isset($this->hookList[$hookName])
           && is_array($this->hookList[$hookName]))
        {
            ...
            foreach($list as $key => $value)
            {
                ...
                if(file_exists(SYSCP_PATH_BASE.$value['file']))
                {
                    ...
                    $list[$key]['obj']->initialize($this->DatabaseHandler, $this->ConfigHandler, $this, $this->LogHandler, $this->TemplateHandler);
                }
            }
    ...
    public function schedule($file, $class, $method, $params)
    {
        ...
        $query = sprintf($query, TABLE_PANEL_TASKS, $file, $class, $method, urlencode(serialize($params)));
