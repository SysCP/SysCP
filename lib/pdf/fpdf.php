...
*******************************************************************************/function FPDF($orientation = 'P', $unit = 'mm', $format = 'A4')
		{
			...
			if(is_string($format))
			{
				...
				if($format == 'a3')$format = array(
					841.89,
					1190.55
				);
				elseif ($format == 'a4')$format = array(
					595.28,
					841.89
				);
				elseif ($format == 'a5')$format = array(
					420.94,
					595.28
				);
				elseif ($format == 'letter')$format = array(
					612,
					792
				);
				elseif ($format == 'legal')$format = array(
					612,
					1008
				);
		...
		function SetDisplayMode($zoom, $layout = 'continuous')
		{
			...
			if($zoom == 'fullpage'
			   ...
			   || $zoom == 'real'
			   || $zoom == 'default'
			   || !is_string($zoom))$this->ZoomMode = $zoom;
			else $this->Error('Incorrect zoom display mode: ' . $zoom);

			if($layout == 'single'
			   || $layout == 'continuous'
			   || $layout == 'two'
			   || $layout == 'default')$this->LayoutMode = $layout;
		...
		function SetDrawColor($r, $g = -1, $b = -1)
		{
			...
			if(($r == 0 && $g == 0 && $b == 0)
			   || $g == -1)$this->DrawColor = sprintf('%.3f G', $r/255);
		...
		function SetFillColor($r, $g = -1, $b = -1)
		{
			...
			if(($r == 0 && $g == 0 && $b == 0)
			   || $g == -1)$this->FillColor = sprintf('%.3f g', $r/255);
		...
		function SetTextColor($r, $g = -1, $b = -1)
		{
			...
			if(($r == 0 && $g == 0 && $b == 0)
			   || $g == -1)$this->TextColor = sprintf('%.3f g', $r/255);
		...
		function GetStringWidth($s)
		{
			...
			function AddFont($family, $style = '', $file = '')
			{
				...
				if($file)
				{
					if($type == 'TrueType')$this->FontFiles[$file] = array(
						'length1' => $originalsize
					);
					else $this->FontFiles[$file] = array(
						'length1' => $size1,
						'length2' => $size2
					);
				}
				...
				if(!isset($this->fonts[$fontkey]))
				{
					...
					if(isset($this->CoreFonts[$fontkey]))
					{
						if(!isset($fpdf_charwidths[$fontkey]))
						{
							...
							if($family == 'times'
							   || $family == 'helvetica')$file.= strtolower($style);
			...
			function Text($x, $y, $txt)
			{
				...
				if($this->underline
				   && $txt != '')$s.= ' ' . $this->_dounderline($x, $y, $txt);
				...
				function Write($h, $txt, $link = '')
				{
					...
					function Image($file, $x, $y, $w = 0, $h = 0, $type = '', $link = '')
					{
						...
						if(!isset($this->images[$file]))
						{
							...
							if($type == 'jpg'
							   || $type == 'jpeg')$info = $this->_parsejpg($file);
