                       ...
                       && $Xml = self::GetXmlFromZip($Filename))
                    {
                        ...
                        foreach($PhpValues as $Value)
                        {
                            if($XmlPhp->
                            {
                                $Value})
                                {
                                    if(self::TrueFalseIniGet($Value) != $XmlPhp->
                                    {
                                        $Value}
                                ...
                                if($XmlPhp->
                                {
                                    'post-max-size'})
                                    {
                                        if(self::PhpMemorySizeToBytes(ini_get('post_max_size')) < intval($XmlPhp->
                                        {
                                            'post-max-size'})
                                        ...
                                        if($XmlPhp->
                                        {
                                            'memory-limit'})
                                            {
                                                if(self::PhpMemorySizeToBytes(ini_get('memory_limit')) < intval($XmlPhp->
                                                {
                                                    'memory-limit'})
                                                ...
                                                if($XmlPhp->
                                                {
                                                    'max-execution-time'})
                                                    {
                                                        if(ini_get('max_execution_time') < intval($XmlPhp->
                                                        {
                                                            'max-execution-time'})
                                                        ...
                                                        if($XmlDb->db->id)
                                                        {
                                                            if($XmlDb->db->
                                                            {
                                                                ...
                                                                if(version_compare($XmlDb->db->
                                                                {
                                                                    'server-min-version'}, mysql_get_server_info()) == 1)
                                                                    {
                                                                                        ...
                                                                                        private function ShowPackageInfo($PackageId, $All = false)
                                                                                        {
                                                                                            ...
                                                                                            if($Xml->
                                                                                            {
                                                                                                'installed-size'})
                                                                                                {
                                                                                                    ...
                                                                                                    $Fieldvalue = 'ca. ' . number_format(intval($Xml->
                                                                                                    {
                                                                                                        'installed-size'})/(1024*1024), 2) . ' MB';
                                                                                                        eval("\$Data.=\"" . getTemplate("aps/data") . "\";");
                                                                                                    }
