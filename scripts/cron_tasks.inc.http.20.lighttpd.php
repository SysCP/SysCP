...
public $mod_accesslog_loaded = "0";
	...
	{
		$query = "SELECT `id`, `ip`, `port`, `listen_statement`, `namevirtualhost_statement`, `vhostcontainer`, "
				." `vhostcontainer_servername_statement`, `specialsettings`, `ssl`, `ssl_cert` "
	...
	protected function createLighttpdHosts($ip, $port, $ssl, $vhost_filename)
	{
		...
		if($ssl == '0')
		{
			$query2 = "SELECT `d`.`id`, `d`.`domain`, `d`.`customerid`, `d`.`documentroot`, `d`.`ssl`, "
							."`d`.`parentdomainid`, `d`.`ipandport`, `d`.`ssl_ipandport`, `d`.`ssl_redirect`, "
                                                        ...
                                                        ."`d`.`safemode`, `d`.`speciallogfile`, `d`.`specialsettings`, `pd`.`domain` AS `parentdomain`, `c`.`loginname`, "
                                                        ...
                                                        ."WHERE `d`.`ipandport`='".$ipandport['id']."' "
							."ORDER BY `d`.`iswildcarddomain`, `d`.`domain` ASC";
		}
		else
		{
			$query2 .= "SELECT `d`.`id`, `d`.`domain`, `d`.`customerid`, `d`.`documentroot`, `d`.`ssl`, "
							."`d`.`parentdomainid`, `d`.`ipandport`, `d`.`ssl_ipandport`, `d`.`ssl_redirect`, "
                                                        ...
                                                        ."FROM `" . TABLE_PANEL_DOMAINS . "` `d` LEFT JOIN `" . TABLE_PANEL_CUSTOMERS . "` `c` USING(`customerid`) "
                                                        ."LEFT JOIN `" . TABLE_PANEL_DOMAINS . "` `pd` ON (`pd`.`id` = `d`.`parentdomainid`) "
                                                        ."WHERE `d`.`ssl_ipandport`='".$ipandport['id']."' "
							."ORDER BY `d`.`iswildcarddomain`, `d`.`domain` ASC";
		}
