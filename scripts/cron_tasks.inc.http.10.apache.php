...
class apache
{
	...
	*/
	...
	{
		$result_ipsandports = $this->db->query("SELECT `ip`, `port`, `listen_statement`, `namevirtualhost_statement`,"
							." `vhostcontainer`, `vhostcontainer_servername_statement`,"
							." `specialsettings`, `ssl`, `ssl_cert`"
							." FROM `" . TABLE_PANEL_IPSANDPORTS . "` ORDER BY `ip` ASC, `port` ASC");
	...
	*/
	...
	{
		if( $ssl_vhost === true && $domain['ssl'] != '1' )
		{
			return '';
		}
		...
		if(preg_match('/^https?\:\/\//', $domain['documentroot']) && $ssl_vhost == false )
		{
			$vhost_content .= '  Redirect / ' . $domain['documentroot'] . "\n";
		}
		else
		{
			if( $ssl_vhost === true && $domain['ssl'] == '1' && $domain['ssl_cert'] != '' )
			{
	...
	*/
	...
	{
		$result_domains = $this->db->query("SELECT `d`.`id`, `d`.`domain`, `d`.`customerid`, `d`.`documentroot`, `d`.`ssl`, "
							."`d`.`parentdomainid`, `d`.`ipandport`, `d`.`ssl_ipandport`, `d`.`ssl_redirect`, "
							."`d`.`isemaildomain`, `d`.`iswildcarddomain`, `d`.`wwwserveralias`, `d`.`openbasedir`, `d`.`openbasedir_path`, "
							."`d`.`safemode`, `d`.`speciallogfile`, `d`.`specialsettings`, `pd`.`domain` AS `parentdomain`, `c`.`loginname`, `d`.`phpsettingid`, `c`.`adminid`, "
							."`c`.`guid`, `c`.`email`, `c`.`documentroot` AS `customerroot`, `c`.`deactivated`, `c`.`phpenabled` AS `phpenabled` "
							."FROM `" . TABLE_PANEL_DOMAINS . "` `d` LEFT JOIN `" . TABLE_PANEL_CUSTOMERS . "` `c` USING(`customerid`) "
							."LEFT JOIN `" . TABLE_PANEL_DOMAINS . "` `pd` ON (`pd`.`id` = `d`.`parentdomainid`) "
							."WHERE `d`.`aliasdomain` IS NULL ORDER BY `d`.`iswildcarddomain`, `d`.`domain` ASC"
							);
