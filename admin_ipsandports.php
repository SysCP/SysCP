   ...
   || $page == 'overview')
{
	if($action == '')
	{
		...
		$ipsandports = '';
		$result = $db->query("SELECT `id`, `ip`, `port`, `vhostcontainer`, `specialsettings` FROM `" . TABLE_PANEL_IPSANDPORTS . "` " . $paging->getSqlWhere(false) . " " . $paging->getSqlOrderBy() . " " . $paging->getSqlLimit());
