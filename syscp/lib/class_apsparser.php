...
class ApsParser
{
	...
	private function ManagePackages()
	{
		...
		if(isset($_POST['save']))
		{
			...
			else
			{
				...
				if($RemoveIds != '' && !isset($_POST['answer']))
				{
					//show yes/no question
					$Output  = $lng['question']['reallyremovepackages'];
		...
		if($FilesSet == true)
		{
			...
			foreach($Files as $File)
			{
				if(isset($_FILES[$File]))
				{
					...
					if(substr($_FILES[$File]['name'], -3) != 'zip' && $_FILES[$File]['error'] == 0)
					{
						$Errors[] = $lng['aps']['notazipfile'];
					}

					if(($_FILES[$File]['size'] > self::PhpMemorySizeToBytes(ini_get('upload_max_filesize')) && $_FILES[$File]['error'] == 0) || $_FILES[$File]['error'] == 1)
					{
	...
	*/
	private function SearchPackages()
	{
		...
		if(isset($_GET['keyword']) && preg_match('/^[- _0-9a-z\.,הצü:;]+$/i', $_GET['keyword']) != false)
		{
			//split all keywords
			$Elements = split('[ ,;]', trim($_GET['keyword']));

			if(count($Elements) == 1 && strlen($Elements[0]) == 0)
			{
		...
		else if(isset($_GET['keyword']) && strlen($_GET['keyword']) != 0)
		{
		...
		$Keyword = '';
		if(isset($_GET['keyword']) && $Error == 0)$Keyword = htmlspecialchars($_GET['keyword']);
	...
	*/
	private function InstallNewPackage($Filename)
	{
		...
		if(file_exists($Filename) && $Xml = self::GetXmlFromZip($Filename))
		{
			$Error = array();
			...
			if($XmlPhp->extension)
			{
				...
				{
					if(!in_array($Extension, $ExtensionsLoaded) && !self::CheckException('php', 'extension', $Extension))
					{
			...
			if($XmlPhp->function)
			{
				...
				{
					if(!function_exists($Function) && !self::CheckException('php', 'function', $Function))
					{
			...
			//php values
			$PhpValues = array(	'short-open-tag',
								'file-uploads',
								'magic-quotes-gpc',
								'register-globals',
								'allow-url-fopen',
			...
			foreach($PhpValues as $Value)
			{
				if($XmlPhp->{$Value})
				{
					if(self::TrueFalseIniGet($Value) != $XmlPhp->{$Value} && !self::CheckException('php', 'configuration', str_replace(array('-'), array('_'), $Value)))
					{
			...
			{
				if(self::PhpMemorySizeToBytes(ini_get('post_max_size')) < intval($XmlPhp->{'post-max-size'}) && !self::CheckException('php', 'configuration', 'post_max_size'))
				{
			...
			{
				if(self::PhpMemorySizeToBytes(ini_get('memory_limit')) < intval($XmlPhp->{'memory-limit'}) && !self::CheckException('php', 'configuration', 'memory_limit'))
				{
			...
			{
				if(ini_get('max_execution_time') < intval($XmlPhp->{'max-execution-time'}) && !self::CheckException('php', 'configuration', 'max_execution_time'))
				{
			...
			if($XmlAsp->handler || $XmlAsp->permissions || $XmlAsp->version)
			{
				$Error[] = $lng['aps']['asp_net'];
			}
			...
			if($XmlWebserver->{'required-module'})
			{
				if(function_exists('apache_get_modules'))
				{
					...
					{
						if(!in_array($Module, $ModulesLoaded) && !self::CheckException('webserver', 'module', $Module))
						{
			...
			//webserver .htaccess
			if($XmlWebserver->htaccess && !self::CheckException('webserver', 'htaccess', 'htaccess'))
			{
				$Error[] = $lng['aps']['webserver_htaccess'];
			}

			//configuration script check
			if($Xml->{'configuration-script-language'} && $Xml->{'configuration-script-language'} != 'php')
			{
			...
			else
			{
				...
				//copy license
				if($Xml->license && $Xml->license->text->file)
				{
	...
	*/
	public function MainHandler($Action)
	{
		...
		if(!is_writable('./temp/') || !is_writable('./packages/'))
		{
		...
		else if($Action == 'remove')
		{
			...
			{
				if(isset($_GET['answer']) && $_GET['answer'] == $lng['panel']['yes'])
				{
		...
		else if($Action == 'manageinstances')
		{
			self::ManageInstances();
		}
		else if($Action == 'managepackages')
		{
			self::ManagePackages();
		}
	...
	*/
	private function ValidatePackageData($PackageId, $CustomerId)
	{
		...
		foreach($Xml->settings->group as $Group)
		{
			foreach($Group->setting as $Setting)
			{
				...
				if($Setting['type'] == 'string' || $Setting['type'] == 'password')
				{
					...
					{
						if(isset($Setting['min-length']) && strlen($_POST[$FieldId]) < $Setting['min-length'])
						{
							if(!in_array($FieldId, $Error))$Error[] = $FieldId;
						}

						if(isset($Setting['max-length']) && strlen($_POST[$FieldId]) > $Setting['max-length'])
						{
							if(!in_array($FieldId, $Error))$Error[] = $FieldId;
						}

						if(isset($Setting['regex']) && !preg_match("/" . $Setting['regex'] . "/", $_POST[$FieldId]))
						{
							if(!in_array($FieldId, $Error))$Error[] = $FieldId;
						}
						/*
				...
				else if($Setting['type'] == 'domain-name')
				{
					if(isset($_POST[$FieldId]))
					{
				...
				else if($Setting['type'] == 'integer')
				{
					if(isset($_POST[$FieldId]))
					{
						...
						if(isset($Setting['min']) && intval($_POST[$FieldId]) < intval($Setting['min']))
						{
							if(!in_array($FieldId, $Error))$Error[] = $FieldId;
						}

						if(isset($Setting['max']) && intval($_POST[$FieldId]) > intval($Setting['max']))
						{
							if(!in_array($FieldId, $Error))$Error[] = $FieldId;
						}
				...
				else if($Setting['type'] == 'float')
				{
					if(isset($_POST[$FieldId]))
					{
						...
						if(isset($Setting['min']) && floatval($_POST[$FieldId]) < floatval($Setting['min']))
						{
							if(!in_array($FieldId, $Error))$Error[] = $FieldId;
						}

						if(isset($Setting['max']) && floatval($_POST[$FieldId]) > floatval($Setting['max']))
						{
							if(!in_array($FieldId, $Error))$Error[] = $FieldId;
						}
				...
				{
					if(isset($_POST[$FieldId]) && $_POST[$FieldId] == 'true')
					{
		...
		if($Xml->license)
		{
			if($Xml->license['must-accept'] && $Xml->license['must-accept'] == 'true')
			{
				if(isset($_POST['license']) && $_POST['license'] == 'true')
				{
	...
	*/
	private function ShowPackageInstaller($PackageId, $WrongData, $CustomerId)
	{
		...
		if(!in_array('main_location', $WrongData) && !in_array('main_domain', $WrongData))$Temp .= '<br/>';
		...
		foreach($Xml->settings->group as $Group)
		{
			...
			foreach($Group->setting as $Setting)
			{
				...
				if($Setting['type'] == 'string' || $Setting['type'] == 'email' || $Setting['type'] == 'integer' || $Setting['type'] == 'float' || $Setting['type'] == 'domain-name')
				{
					if(!$GroupPrinted)
					{
				...
				else if($Setting['type'] == 'boolean')
				{
					...
					else if($Setting['default-value'] == 'true')
					{
						$Temp .= '<input checked="checked" name="' . $Setting['id'] . '" type="checkbox" value="true" />';
					}
					else
				...
				else if($Setting['type'] == 'enum')
				{
					...
					foreach($Setting->choice as $Choice)
					{
						...
						else if(strval($Choice['id']) == strval($Setting['default-value']))
						{
							$Temp .= '<option selected="selected" value="' . $Choice['id'] . '">' . $Choice->name . '</option>';
						}
						else
